  ...
  return (
    <section className="card guide" style={{ "--stage": meta.color } as CSSProperties}>
      <div className="section-title">
        <b>{HAZARD_LABEL[hazard]}</b> 단계별 예방 조치사항
      </div>

      <div className="steps" role="tablist" aria-label="단계 선택">
        {ORDER.map((lv) => {
          const m = content[lv];
          return (
            <button
      ...
      <div className="guide__body">
        ...
        {meta.draft && (
          <p className="draft-note">※ {HAZARD_LABEL[hazard]} 조치문구는 초안입니다(안전보건처 검토 예정).</p>
        )}
      </div>
    </section>
  );
