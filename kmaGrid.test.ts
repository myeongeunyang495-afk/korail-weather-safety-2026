...
import { buildNationwideAlerts, type NationwideAlert } from "../lib/nationwideAlerts";
import {
  classifyCold,
  classifyHeat,
  classifyRain,
...
export type { NationwideAlert } from "../lib/nationwideAlerts";
...
export function useReading() {
  ...
  const [nationwideAlerts, setNationwideAlerts] = useState<NationwideAlert[]>([]);
  ...
  const queryByCoords = useCallback(
    async (lat: number, lon: number, label: string, mode: ViewMode = "auto") => {
      ...
      try {
        ...
        buildNationwideAlerts(provider, r.observedAt)
          .then((alerts) => {
            if (id === reqId.current) setNationwideAlerts(alerts);
          })
          .catch(() => {
            if (id === reqId.current) setNationwideAlerts([]);
          });
      } catch {
  ...
  return { reading, hourly, nationwideAlerts, loading, error, queryByCoords, queryByGps, refresh };
}
