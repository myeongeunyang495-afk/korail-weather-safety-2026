  ...
  return hourly.map((h, i) => {
    ...
    return (
      <g key={i}>
        ...
        <text x={i * W + W / 2} y={190} className="forecast__hr">
          {h.time.getHours()}시
        </text>
      </g>
    );
...
function nationalValueLabel(hazard: HazardKind, value: number) {
  if (hazard === "heat" || hazard === "cold") return `${value.toFixed(1)}°C`;
  return `${value < 10 ? value.toFixed(1) : Math.round(value)}${hazard === "snow" ? "cm" : "mm"}`;
}

function NationalAlertDetail({ hazard, alerts }: { hazard: HazardKind; alerts: NationwideAlert[] }) {
  const alert = alerts.find((item) => item.hazard === hazard);
  const regions = alert?.regions ?? [];
  return (
    <div className="card national-detail">
      <h3>전국 {HAZARD_LABEL[hazard]} 특보 발령 예상 지역</h3>
      {regions.length === 0 ? (
        <p className="national-detail__empty">현재 예보 기준 특보 발령 예상 지역이 없습니다.</p>
      ) : (
        <ul className="national-detail__list">
          {regions.map((region) => (
            <li key={`${region.location}-${region.time.toISOString()}`} style={{ "--stage": STAGE_VAR[region.level] } as CSSProperties}>
              <b>{region.location}</b>
              <span>{region.time.getMonth() + 1}/{region.time.getDate()} {region.time.getHours()}시 · {HAZARD_LABEL[hazard]}{region.level === "danger" ? "경보" : "주의보"}</span>
              <em>{nationalValueLabel(hazard, region.value)}</em>
            </li>
          ))}
        </ul>
      )}
...
export function HourlyForecast({ hourly, reading, loading, hazardOverride, nationwideAlerts }: Props) {
  ...
  return (
    <section className="forecast">
      ...
      <div className="forecast__chartwrap">
        ...
        <svg className="forecast__chart" viewBox={`0 0 ${hourly.length * W} 200`} preserveAspectRatio="none" role="img" aria-label={`시간대별 ${title}`}>
          {renderBars(hourly, hazard)}
        </svg>
      </div>

      <NationalAlertDetail hazard={hazard} alerts={nationwideAlerts} />
