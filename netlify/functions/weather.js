...
function isoFromKma(t) {
  // t = YYYYMMDDHHmm, KMA forecast time is Korea Standard Time.
...
function baseCandidates(now, hoursBack, kind) {
  // Netlify runs on UTC. KMA base_date/base_time must be generated in KST.
  const base = new Date(now.getTime() + 9 * 3600_000);
  const minuteCut = 45;
  if (base.getUTCMinutes() < minuteCut) base.setUTCHours(base.getUTCHours() - 1);
  const mm = kind === "fcst" ? "30" : "00";
  return Array.from({ length: hoursBack + 1 }, (_, i) => {
    const c = new Date(base);
    c.setUTCHours(base.getUTCHours() - i);
    return {
