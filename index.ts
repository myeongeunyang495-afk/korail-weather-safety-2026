...
function activeHazardsFor(date: Date): HazardKind[] {
  const month = date.getMonth() + 1;
  return month >= 4 && month <= 11 ? ["heat", "rain"] : ["snow", "cold"];
}
...
export async function buildNationwideAlerts(provider: WeatherProvider, 기준시각: Date = new Date()): Promise<NationwideAlert[]> {
  const hazards = activeHazardsFor(기준시각);
